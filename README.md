...
- **Live 3D background across every page.** A layered WebGL backdrop renders behind every route: an undulating point-field energy wave, a slowly rotating constellation of linked nodes, drifting wireframe hologram shards, and crossing foreground particles — all Points/basic materials (no lights or post-processing, ~8 draw calls). The camera breathes and follows the mouse for parallax; live telemetry drives the scene (incidents churn the wave faster and fire expanding pulse rings); every layer is scaled to the camera frustum so nothing clips at any screen size; the loop pauses when the tab is hidden and respects `prefers-reduced-motion`.
